...
#### Browser download
On managed devices the browser download is blocked, so it must be downloaded manually:
...
Playwright for java uses a temp directory to launch the browser, which is also blocked if it is not in `c:\dev`
